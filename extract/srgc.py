    ...
    def execute(self):
        def check_obj_no_mat():
            ...
            if read_str(f, 4) == "NGOB":  # means there is no material block
                ...
                for _ in range(material_count):
                    ...
                    if a:  # if theres a texture count the file uses textures without a texture block
                        return True
                    f.seek(4, 1)
            return False
            ...
            for sf_count, sf_pos, sf_type in zip(
                ...
                for i, j in zip(sf_offsets, sf_offsets[1:]):
                    ...
                    if read_str(f, 4) == "NGIF":
                        ...
                        if f.tell() < os.path.getsize(self.file) and read_str(f, 4) == "GCIX":
                            f.seek(i)
                            ...
                            if type_byte:
                                ...
                                obj_need_texture = []
                            continue
                        elif start_int == 1195573248:
                            file_name = "Unnamed_File_" + str(index) + ".SonicRiders_G.pathfinding"
                            index += 1
                        elif start_int == 16926531:
                            file_name = "Unnamed_File_" + str(index) + ".SonicRiders_G.splines"
                            index += 1
                        elif start_int == 16842753:
                            file_name = "Unnamed_File_" + str(index) + ".SonicRiders_G.collision"
                            index += 1
                        elif start_int >> 24 & 128:
                            file_name = "Unnamed_File_" + str(index) + ".SonicRiders_G.objects"
                            index += 1
                        elif 1 < start_int < 25:  # yeah i have no idea im just guessing what a good range would be
                            file_name = "Unnamed_File_" + str(index) + ".SonicRiders_G.item_render"
                            index += 1
                        elif start_int == 1:  # its not worth making a function for this
                            file_name = "Unnamed_File_" + str(index) + ".SonicRiders_G.scene_render"
                            index += 1
                        else:
        ...
        if self.set_batch == "Single":
            ...
            for self.file in file_list:
                ...
                run_file()
                self.f.close()
        show_finished("Sonic Riders GC Extractor")
        return {'FINISHED'}
...
class SonicRGCTools(Operator, ImportHelper):
    """Extract files from an uncompressed Sonic Riders GC Archive"""
    bl_idname = "srgc.extract"
    ...
    def draw(self, context):
        layout = self.layout
        box = layout.box()
        box.label(text="Sonic Riders GC Extractor Settings:", icon="KEYFRAME_HLT")
