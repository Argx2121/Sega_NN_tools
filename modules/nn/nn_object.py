...
from .object_assets.nn_util import implicit_faces
from ..util import *
...
class Read:
    ...
    def lno(self) -> ModelData:
        ...
        model.faces = self._execute(data.face_set_offset, 3, faces.Read(f, post, data.face_set_count).type_2)

        model.vertices, model.mesh_info = self._execute(
            data.vertex_buffer_offset, 4, vertices.Read(f, post, self.format_type, data.vertex_buffer_count).type_2)

        print(data.mesh_data_offset)
        model.build_mesh = console_out("Parsing Sub Mesh Data...", meshes.Read(
            f, post, data.mesh_sets_count, data.mesh_data_offset, data.mesh_data_count).type_2)  # seeks in method
    ...
    def sno(self) -> ModelData:
        ...
        model.vertices, model.mesh_info = self._execute(
            data.vertex_buffer_offset, 4, vertices.Read(f, post, self.format_type, data.vertex_buffer_count).type_3)
