...
nv_list_types = [a[1] for a in nv_list[1:]]
selected_file = ""


class ImportSegaNV(bpy.types.Operator, ImportHelper):
    """Import a Sega NN Material animation"""
    bl_idname = "import_nn.sega_nv"
    bl_label = "Import *nv Motion Material"
    bl_options = {'REGISTER', 'UNDO'}
    filename_ext = "*.cnv;*.env;*.gnv;*.inv;*.lnv;*.snv;*.unv;*.xnv;*.znv"
    filter_glob: StringProperty(
        default="*.cnv;*.env;*.gnv;*.inv;*.lnv;*.snv;*.unv;*.xnv;*.znv",
        options={'HIDDEN'},
        maxlen=255)
    ...
    def draw(self, context):
        ...
        if selected_file != bpy.context.space_data.params.filename:
            ...
            if len(selected_file) > 2 and selected_file[-3:].upper() in nv_list_types:
    ...
    def execute(self, context):
        ...
        if nn_format != "Match__":
            ...
            pass  # handled later
        return mat_anim_import(self.filepath, settings)


def menu_func_import(self, context):  # add to dynamic menu
    self.layout.operator(ImportSegaNV.bl_idname, text="Sega NN Motion Material (.xnv, .znv, etc.)")
...
def mat_anim_import(filepath, settings):
    def execute_set(file_path):
        ...
        if block == expected_block:
            ...
            if nn.animation:
                print("Making Material Animations----------------------------")
                message = "Making" + " " + nn.name
                start_time = console_out_pre(message)
                Animation(nn, settings).material_animation()
                console_out_post(start_time)
        else:
            show_not_read("NN Motion Material Importer")
        f.close()

    def execute_match(file_path):
        if not file_path.lower().endswith("nv"):
            return
        f = open(file_path, 'rb')
        block = read_str_nulls(f, 4)[0]
        f.seek(0)
    ...
    if settings.format != "Match__":
        name_require = "." + settings.format[-1].lower() + "nv"
