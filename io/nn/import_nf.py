...
class ImportSegaNF(bpy.types.Operator, ImportHelper):
    """Import a Sega NN Morph animation"""
    bl_idname = "import_nn.sega_nf"
    bl_label = "Import *nf Motion Morph"
    bl_options = {'REGISTER', 'UNDO'}
    filename_ext = "*.cnf;*.enf;*.gnf;*.inf;*.lnf;*.snf;*.unf;*.xnf;*.znf"
    filter_glob: StringProperty(
        default="*.cnf;*.enf;*.gnf;*.inf;*.lnf;*.snf;*.unf;*.xnf;*.znf",
        options={'HIDDEN'},
        maxlen=255)
    ...
    def execute(self, context):
        ...
        if nn_format != "Match__":
            ...
            pass  # handled later
        return morph_anim_import(self.filepath, settings)


def menu_func_import(self, context):  # add to dynamic menu
    self.layout.operator(ImportSegaNF.bl_idname, text="Sega NN Motion Morph (.xnf, .znf, etc.)")
...
def morph_anim_import(filepath, settings):
    def execute_set(file_path):
        ...
        if block == expected_block:
            ...
            if nn.animation:
                print("Making Morph Animations---------------------------")
                message = "Making" + " " + nn.name
                start_time = console_out_pre(message)
                Animation(nn, settings).morph_animation()
                console_out_post(start_time)
        else:
            show_not_read("NN Motion Morph Importer")
        f.close()

    def execute_match(file_path):
        if not file_path.lower().endswith("nf"):
            return
        f = open(file_path, 'rb')
        block = read_str_nulls(f, 4)[0]
        f.seek(0)
    ...
    if settings.format != "Match__":
        name_require = "." + settings.format[-1].lower() + "nf"
